...
export default function Chat() {
  ...
  const [receiverId, setReceiverId] = useState(""); // ✅ Input ilə dəyişəcək
  ...
  const sendMessage = async () => {
    if (connection && message.trim() !== "" && receiverId.trim() !== "") {
      try {
        console.log("Send",userId,receiverId,message);
        await connection.invoke("SendMessage", userId, receiverId, message);
 
        await fetch("https://localhost:7298/api/Chat/send", {
          method: "POST",
          headers: {
